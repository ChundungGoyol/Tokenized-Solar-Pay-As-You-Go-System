...
(define-constant err-listing-not-found (err u118))
(define-constant err-insufficient-listing (err u119))
(define-constant err-own-listing (err u120))

(define-map credit-listings uint { seller: principal, amount: uint, price: uint, active: bool })
...
(define-public (list-credits (amount uint) (price uint))
    (let ((sender tx-sender)
        ...
        (asserts! (>= balance amount) err-insufficient-balance)
        (map-set user-balances sender (- balance amount))
        (map-set credit-listings listing-id { seller: sender, amount: amount, price: price, active: true })
...
(define-public (buy-credits (listing-id uint))
    (let ((buyer tx-sender)
        ...
        (asserts! (not (is-eq buyer seller)) err-own-listing)
        (asserts! (>= buyer-balance price) err-insufficient-balance)
        (try! (stx-transfer? price buyer seller))
        (map-set user-balances buyer (- buyer-balance price))
        (map-set user-balances seller (+ (default-to u0 (map-get? user-balances seller)) price))
        (map-set user-balances buyer (+ buyer-balance amount))
...
(define-public (cancel-listing (listing-id uint))
    (let ((sender tx-sender)
          ...
          (seller (get seller listing))
          (amount (get amount listing)))
        (asserts! (is-eq sender seller) err-owner-only)
        (asserts! (get active listing) err-listing-not-found)
        (map-set user-balances sender (+ (default-to u0 (map-get? user-balances sender)) amount))
        (map-set credit-listings listing-id (merge listing { active: false }))
        (ok true)))
